...
|----|---------|-----|-----|--------|--------|----------|
| VRD-D016 | Tests tab empty in Xray Test Set (Cloud internal model) | S3 | P2 | product/xray | **Open** | — |
| VRD-D015 | Xray step API — steps not appearing in Test Details on Cloud | S3 | P2 | product/xray | **Open** | — |
...
### VRD-D016 — Tests tab empty in Xray Test Set (Cloud)
- **Severity:** S3 Medium · **Priority:** P2 · **Status:** Open
- **Module:** product/xray
- **Description:** After "Push All with Group into Test Set", the Xray Test Set issue
  is created and test cases are individually pushed, but the **Tests tab** inside the
  Test Set remains empty. Confirmed on Xray Cloud (e.g. APR-43).
- **Root cause:** Xray Cloud stores the Test→TestSet relationship in its own internal
  data model, not as standard Jira issue links. The Server REST API
  (`/rest/raven/1.0/api/testset/{key}/test`) returns a non-success status on Cloud.
  Standard `issueLink` fallback creates visible links in Jira but does not populate
  the Xray Tests tab.
- **Resolution plan:** Implement Xray Cloud v2 API (`https://xray.cloud.getxray.app/api/v2/`)
  which requires a separate Xray API client ID + client secret (not the Jira API token).
  User must generate these from their Xray Cloud settings → "API Keys". Tracked in S7.

### VRD-D015 — Xray step API — steps not appearing in Test Details
- **Severity:** S3 Medium · **Priority:** P2 · **Status:** Open
- **Module:** product/xray
- **Description:** Test steps generated by AI are sent to Xray via
  `POST /rest/raven/1.0/api/test/{key}/step` but the **Test Details** tab in Xray
  shows "There are no steps defined". Confirmed on Xray Cloud (e.g. APR-47).
- **Root cause:** Two body formats tried (`{"step":…}` and `{"action":…}`) — neither
  confirmed working on Xray Cloud. The Xray Cloud v2 step API at
  `https://xray.cloud.getxray.app/api/v2/test/{issueId}/step` requires the Jira issue ID
  (numeric) and a separate OAuth2 token, not the REST v1 endpoint.
- **Workaround:** Steps are included in the Jira description as structured markdown.
  Users can manually copy steps into Xray Test Details.
- **Resolution plan:** Same as VRD-D016 — Xray Cloud v2 API implementation in S7.
